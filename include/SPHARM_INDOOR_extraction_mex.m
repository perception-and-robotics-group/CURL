function [lidarConfig,coefficient_r_cell,mask_update,true_point_cloud_original,square_grid,points_from_mesh,Extraction_time] = SPHARM_INDOOR_extraction_mex(varargin)
% Read parameters
...
% Procedure    
    % generate the mask
    [square_grid,~] = SPHARM_point_cloud_initialization_INDOOR(inputPointCloud,lidarConfig); % revise this function to make the input has lidarConfig
    inputPointCloud = outlier_filter(square_grid.point_cloud,1,7);
    [square_grid,~] = SPHARM_point_cloud_initialization_INDOOR(inputPointCloud,lidarConfig); % revise this function to make the input has lidarConfig
    ...
    if (lidarConfig.is_density_enhance)
        if ~exist(original_sampled_file,'file')
            ...
            [sampled_points, ~, ~,~] = intersection_between_faces_lines_grid_mex(non_zero_data,faces,table_init_grid,recons_table_check);
        else
    ...
    if isFigure == true
        ...
        title('Denoised Sampled Point Cloud');
        ...
        title('Denosied Sampled Point Cloud');
    end
end